 ...
 * creates a {@link XTFQuery} object that can be executed against the 
 * resource index.
 *
 ...
 * basis. Once the translation is finished the {@link XTFQuery} object
 * returned by {@link #createXTFQuery} will contain the list of pid along 
 * with full paths as a result of the query.
 */
public class XTFQueryBuilder implements QueryNodeVisitor {

    /**
     * log4j logger for this class.
     */
    private static final Logger log = 
		LoggerFactory.getLogger(XTFQueryBuilder.class);
    ...
    /**
     * Creates a new <code>XTFQueryBuilder</code> instance.
     *
     ...
     */
    private XTFQueryBuilder(QueryRootNode root,
                               SessionImpl session,
     ...
     *                        information.
     * @return the XTFQuery object.
     * @throws RepositoryException if an error occurs during the translation.
     */
    public static XTFQuery createQuery(QueryRootNode root,
										  SessionImpl session,
										  ItemStateManager sharedItemMgr,
										  NamespaceMappings nsMappings,
										  PropertyTypeRegistry propReg
            ...
            throws RepositoryException 
	{
		...
		XTFQuery q = builder.createXTFQuery();
     ...
     * {@link edu.northwestern.jcr.adapter.xtf.query.XTFQuery}.
     *
     * @return the <code>XTFQuery</code> object.
     * @throws RepositoryException
     */
    private XTFQuery createXTFQuery() throws RepositoryException {
		return (XTFQuery) root.accept(this, null);
    }
    ...
    public Object visit(QueryRootNode node, Object data) 
		throws RepositoryException 
	{
		XTFQuery wrapped = null;
        if (node.getLocationNode() != null) {
			wrapped = (XTFQuery) node.getLocationNode().accept(this, root);
        }
    ...
    public Object visit(TextsearchQueryNode node, Object data) 
	{
		...
		String fieldName = propName.getLocalName();
		String textsearch = node.getQuery();

		Scanner scanner = new Scanner(textsearch);
		String xmlQuery = "";
		while (scanner.hasNext()) {
			xmlQuery += "<term field=\"" + fieldName + "\">"
				+ scanner.next() + "</term>";
		}
		...
		throws RepositoryException 
	{
		...
		for (int i = 0; i < steps.length; i++) {
			...
			if (steps[i] instanceof DerefQueryNode) {
				...
				query.addStep(nameURI, XTFQuery.DEREF,
							  propertyURI);
				try {
					query.execute();
				} catch (Exception e) {
			...
			if (nameTest != null &&
				...
				// jcr:root
				query.addStep("/", XTFQuery.EXACT, null);
				continue;
			}
			...
			if (pred != null) {
				...
				for (int j = 0; j < pred.length; ++j) {
					...
					xmlQuery = (String) pred[j].accept(this, null);
					...
					// filter += s[1];
					// 
					// System.out.println("field: " + s[0]);
					// 
					// Scanner scanner = new Scanner(s[1]);
					// 
					// xmlQuery += "<exact>";
					// while (scanner.hasNext()) {
					// 	xmlQuery += "<term field=\"" + s[0] + "\">"
					// 		+ scanner.next() + "</term>";
					// }
					// 
					// xmlQuery += "</exact>";
			...
			query.addStep(name, type, xmlQuery); // filter);
			try {
				query.execute();
			} catch (Exception e) {
				String msg = "error executing query";
		...
		throws RepositoryException 
	{
		...
		Scanner scanner = new Scanner(stringValues[0]);

		xmlQuery = "<exact>";
		while (scanner.hasNext()) {
			xmlQuery += "<term field=\"" + localName + "\">"
				+ scanner.next() + "</term>";
		}
	 ...
	 * {@link edu.northwestern.jcr.adapter.xtf.query.SearchIndex}.
	 *
	 * @param node the query node
	 * @param data not used in this implementation
	 */
