...
public class QueryResultImpl implements QueryResult {
	...
	/**
	 * The XTFQuery instance.
	 */
	protected final XTFQuery query;
     ...
     *                        result.
     * @param query           the Fedora query to execute on the index.
    ...
    public QueryResultImpl(SearchIndex index,
        ...
        getResults(docOrder ? Integer.MAX_VALUE : Integer.MAX_VALUE);
    }
    ...
    private ScoreNodeIterator getNodeIterator() {
        if (docOrder) {
            return null; // new DocOrderNodeIteratorImpl(itemMgr, resultNodes, 0);
        } else {
    ...
    private boolean isAccessGranted(ScoreNode[] nodes)
        ...
        for (int i = 0; i < nodes.length; i++) {
            try {
                ...
                //     return false;
                // }
				log.debug("checking privilege: " + nodes[i].getNodePath());
				System.out.println("fetching " + nodes[i].getNodePath());
				if (nodes[i] != null && !accessMgr.canRead(PathFactoryImpl.getInstance().create(nodes[i].getNodePath()))) {
        ...
        private void fetchNext() {
            ...
            while (next == null) {
                if (nextPos >= resultNodes.size()) {
                    ...
                    try {
                        ...
                        if (resultNodes.size() == 0) {
                            num = 0; // index.getResultFetchSize();
                        } else {
                ...
                try {
					...
					log.info("fetching " + sn[selectorIndex].getNodePath());
					// System.out.println("fetching " + sn[selectorIndex].getNodePath());
					next = (NodeImpl) itemMgr.getNode(PathFactoryImpl.getInstance().create(sn[selectorIndex].getNodePath()));
					log.info("name of fetched node: " + 
							 next.getQName().getLocalName());
                } catch (RepositoryException e) {
                    System.err.println("Exception retrieving Node with UUID: "
