 ...
 * @version 27 03 2010 11:46:29 AM
 */
...
class JSONValue(val value: JSONSerializable) extends PropertyValue {

  def setIn(json_object: JSONObject, name: String) {
    json_object.put(name, value.toJSON)
  }

  def addTo(json_array: JSONArray) {
    json_array.put(value.toJSON)
  }
}