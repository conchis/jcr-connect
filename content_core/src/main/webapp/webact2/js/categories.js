...
webact.in_package("categories", function (categories) {
	...
	// Counter used to generate uneque ids
	var counter = 0;
	...
	categories.makeCategory = function (options) {
		...
		// Returns the depth of this node (depth 0 = root node)
		that.depth = function () {
			var examine = this;
			var depth = 0;
			while (!examine.isRoot()) {
				depth += 1;
				examine = examine.getParent();
			}
			return depth;
		}
	...
	categories.makeTaxonomy = function (options) {
